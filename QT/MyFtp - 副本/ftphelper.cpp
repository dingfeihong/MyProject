...
HANDLE FtpHelper::CreatFile(char *file){
    ...
    MultiByteToWideChar(CP_ACP,0,c_str2,strlen(c_str2)+1,wszClassName,
...
HANDLE FtpHelper::OpenFile(char* filepath){
    WCHAR wszClassName[256];
    memset(wszClassName,0,sizeof(wszClassName));
    MultiByteToWideChar(CP_ACP,0,filepath,strlen(filepath)+1,wszClassName,
    sizeof(wszClassName)/sizeof(wszClassName[0]));

    HANDLE hFile = CreateFile(wszClassName, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ,
                           NULL, CREATE_ALWAYS, 0, NULL);
    if (INVALID_HANDLE_VALUE == hFile)
    {
        msg->append("open file error");
        return NULL;
    }
    return hFile;
}

int FtpHelper::file_copy(int srcfd,HANDLE destfd,int *psize){
    char* buf;
    char* ptr;
    DWORD dwWrite;
    ...
    *psize = 0;
    ...
    buf = (char*)malloc(sizeof(char) * RECV_BUF_LEN);
    memset(buf, 0, sizeof(char) * RECV_BUF_LEN);
    while (0 != (read_size = recv(srcfd, buf, sizeof(char) * RECV_BUF_LEN,0)))
    {
        if (read_size > 0)
        {
            WriteFile((HANDLE)destfd, buf, read_size, &dwWrite, NULL);
...
void FtpHelper::DownLoad(char *remote_file){
    if(!lock){
        msg->append("上一个命令尚未完成");
        return;
    }
   ...
   /* if((SendControl("CWD",remote_file))&&(RecvControl("250"))){
        ListFile(NULL);
        msg->append("跳转地址到"+QString(QLatin1String(remote_file)));
        return;
    }*/
    if((!SendControl("RETR",remote_file))||(!RecvControl("150","125"))){
        err=WSAGetLastError();
    ...
    if(!(file=CreatFile(remote_file))){
        return;
    }
    //命令套接字已经发送命令，且已经接受到服务器返回的成功信息，接着在数据套接字中获取数据
    if(file_copy(data_sock,file,&numbytes)==-1){
        msg->append("下载失败");
        CloseHandle(file);
        return;
    }
...
void FtpHelper::UpLoad(char* filepath){
    ...
    HANDLE hFile=OpenFile(filepath);
    if(!hFile)
        return;
    if(!Pasv())
        return;
    ...
    if((!SendControl("STOR",tmp+1))||(!RecvControl("150","125"))){
        err=WSAGetLastError();
    ...
    do
    {
        result = ReadFile(hFile, SendBuf, RECV_BUF_LEN, &dwRead, NULL);
        if (result > 0 && dwRead > 0)
        result = send(data_sock, SendBuf, dwRead, 0);
    } while (dwRead > 0);
    msg->append("上传完毕");
    lock=true;
    CloseHandle(hFile);
