    ...
    public partial class fTestBulk : Form
    {
        ...
        private void cmdRead_Click(object sender, EventArgs e)
        {
            ...
            if ((eReturn = mEpReader.Read(readBuffer, 1000, out uiTransmitted)) == ErrorCode.None)
            {
                ...
                if (chkLogToFile.Checked && mLogFileStream != null) //存入文件中
                {
                    if (ckShowAsHex.Checked)
                    {
                        // get the bytes as a hex string
                        StringBuilder sb = GetHexString(readBuffer, 0, uiTransmitted);
                        ...
                        mLogFileStream.Write(readBuffer, 0, uiTransmitted);
                }
        ...
        private void OnDataReceived(object sender, EndpointDataEventArgs e)
        {
            ...
            //int temp0 = 0, temp1 = 0;
            //int uiTransmitted;
            //uiTransmitted = data.Length;
            //if (uiTransmitted > 0)
            //{
            //    tsStatus.Text = uiTransmitted + " bytes read.";
            //    showBytes(data, uiTransmitted);
            //    for (j = 0; j < 10; j++)
            //    {
            //        for (i = 0; i < 15; i++)
            //        {
            //            temp0 = data[i * 4 + 4 + j * 64];
            //            temp1 = data[i * 4 + 1 + 4 + j * 64];
            //            ReadBuffer1[i + j * 15] = temp0 * 256 + temp1;

            //            temp0 = data[i * 4 + 2 + 4 + j * 64];
            //            temp1 = data[i * 4 + 3 + 4 + j * 64];
            //            ReadBuffer2[i + j * 15] = temp0 * 256 + temp1;
            //        }
            //    }
            //    //showBytes(readBuffer0, 32);
            //    showWave(data, uiTransmitted);
            //}
            //else
            //    tsStatus.Text = "No data to read! ";
